    ...
    #Convert the  package to a readable format
        ...
        if protocol == 'ARP' :
             ...
             if ip_ver == 4 :
                   info=package['info']
                   # ipv4头部信息
                   show=show+"IP首部:IP version 4 "+" 头长度:" + str(package['ip_hl'])+" 服务类型："+str(package['ip_tos'])+" 总长度："+ str(package['ip_len'])+" 标识："+str(package['ip_id'])+ \
                   ...
                   info=package['info']
                   # ipv6头部信息
                   show=show+" IP首部:IP version 6 "+"优先级:"+str(info['fc'])+" 流量标识: "+ str(info['flow'])+" 有效载荷长度:"+ str(info['payload_len'])+\
